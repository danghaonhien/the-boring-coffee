...
export default function AdminPage() {
  ...
  const Sidebar = () => (
    <div className="w-64 h-350 bg-gray-800 text-white flex flex-col">
      <div className="p-4 font-bold text-lg">Dashboard</div>
      <button onClick={() => setActiveTab('home')} className={`p-4 text-left ${activeTab === 'home' ? 'bg-gray-700' : ''}`}>Home</button>
      <button onClick={() => setActiveTab('products')} className={`p-4 text-left ${activeTab === 'products' ? 'bg-gray-700' : ''}`}>Products</button>
      <button onClick={() => setActiveTab('orders')} className={`p-4 text-left ${activeTab === 'orders' ? 'bg-gray-700' : ''}`}>Orders</button>
    </div>
  );
  ...
  // Update the return statement to include the sidebar and conditional rendering based on the active tab
  return (
    <div className="flex">
      <Sidebar />
